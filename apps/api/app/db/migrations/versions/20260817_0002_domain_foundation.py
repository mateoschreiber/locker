...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        *audit_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("user_id", "branch_id", "role_id"),
    )
    op.create_table(
        "lockers",
        *audit_columns(),
    ...
    op.create_table(
        ...
        *audit_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("locker_id", "code"),
    )
    op.create_table(
        "locker_locks",
        *audit_columns(),
        sa.Column("compartment_id", sa.UUID(), nullable=False),
        sa.Column("hardware_address", sa.String(120)),
        sa.Column("status", sa.String(32), nullable=False),
        sa.ForeignKeyConstraint(["compartment_id"], ["compartments.id"]),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("compartment_id"),
    )
    op.create_table(
        "cameras",
        *audit_columns(),
    ...
    op.create_table(
        ...
        *audit_columns(),
        ...
        sa.Column("rfid_tag", sa.String(120)),
        sa.Column("status", sa.String(32), nullable=False),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_index(
        "one_active_compartment_placement",
        "tool_placements",
        ["compartment_id"],
        unique=True,
        postgresql_where=sa.text("removed_at IS NULL"),
