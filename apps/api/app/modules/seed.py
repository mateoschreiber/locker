...
def seed_lab(session: Session) -> None:
    ...
    for position in range(1, 25):
        ...
        if compartment is None:
            ...
            session.flush()
        if (
            session.scalar(select(LockerLock).where(LockerLock.compartment_id == compartment.id))
            is None
        ):
        ...
        if tool is None:
            tool = Tool(
                ...
                name=f"Herramienta {position:03d}",
                rfid_tag=f"RFID-{asset_code}",
            )
            session.add(tool)
            session.flush()
        if (
            session.scalar(
                select(ToolPlacement).where(
                    ToolPlacement.tool_id == tool.id, ToolPlacement.removed_at.is_(None)
                )
            )
            is None
        ):
    ...
    for username, display_name, role_code in users:
        ...
        if (
            ...
            session.add(
                Membership(user_id=user.id, branch_id=branch.id, role_id=roles[role_code].id)
            )
    session.commit()
